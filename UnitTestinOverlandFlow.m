...
for i = 2:imax
    ...
    % Drainage Response Time Unit Tests
    L1 = 50;
    L2 = 200;
    L3 = 400;
    Larray1 = ceil(L1/dx);
    ...
    alpha = (1/n) * slope(1).^(1/2);
    
    testtime1 = floor((L1/(alpha*0.000003^(2/3)))^(3/5));
    testtime2 = floor((L2/(alpha*0.000003^(2/3)))^(3/5));
    testtime3 = floor((L3/(alpha*0.000003^(2/3)))^(3/5));
    
    if t(i) == floor(testtime1+1)
        assert(Qtest1(i) >= 0.999*Qtest1(i-1) && Qtest1(i) <= 1.001*Qtest1(i-1), 'code sucks.')
    end
    
    if t(i) == floor(testtime2+1)
        assert(Qtest2(i) >= 0.999*Qtest2(i-1) && Qtest2(i) <= 1.001*Qtest2(i-1), 'code sucks.')
    end
    
    if t(i) == floor(testtime3+1)
        assert(Qtest3(i) >= 0.999*Qtest3(i-1) && Qtest3(i) <= 1.001*Qtest3(i-1), 'code sucks.')
    end
