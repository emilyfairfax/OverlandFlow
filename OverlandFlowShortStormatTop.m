...
for i = 2:imax
    ...
    %Short Storm at Top of Slope
    if t(i)<stormlength % only rain for the length of the storm
        rain(i) = max(stormrainheight.*sin(1.5*3.14*t(i)/(1.5*stormlength)),0); %make the storm intensity go like a cosine, don't let it be negative
        R = zeros(1,N); %base rain array
        R(1:ceil(N/5)) = rain(i); %only rain at top of box
         
    else
        rain(i) = 0; %if its not during the length of the storm there is no rain
        R =rain(i)*ones(1,N); %no rain
    end 
