...
% Constants
    ...
    minutes = 45; %duration of the storm in minutes
    ...
    stormvelocity = .1; %m/s, velocity storm moves down slope
...
for i = 2:imax
    ...
    %Moving Rainstorm
    rain(i) = max(stormrainheight.*cos(2*3.14*t(i)/(2*tmax)),0); %storm loses rainfall intensity with time according to sin function
    R = zeros(1,N); %base rain array
    stormwidth = 100; %width of storm in meters
    stormwidtharray = floor(stormwidth/dx); %find the width of the storm in array elements
    stormtail(i) = ceil(stormvelocity*(t(i))./dx); %find the position of the back of the storm in the array
    stormfront(i) = stormtail(i) + stormwidtharray; %find the position of the front of the storm in the array
    
    startstorm = min(stormtail(i),N); %kill the storm after the Nth position in the array
    endstorm = min(stormfront(i),N); %kill the storm after the Nth position in the array
    
    R(startstorm:endstorm) = rain(i); %put the rainfall intensity in the base rain array only where the storm is located
    ...
    if (rem(t(i),tplot)==0)
        ...
        %Water Thickness Plot
            ...
            ht=text(4/5*xmax,.8,['  ',num2str(floor(round(t(i))/60)), ' minutes  '],'fontsize',18); %print time in animation
            axis([0 xmax 0 1])
